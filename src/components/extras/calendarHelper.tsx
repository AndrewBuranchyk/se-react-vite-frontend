...
export const getLabel = (date: dayjs.ConfigType, viewMode: "month" | "week" | "work_week" | "day" | "agenda") => {
	if (viewMode === Views.MONTH) return dayjs(date).format("MMMM YYYY")
	if (viewMode === Views.WEEK)
		return `${dayjs(date).startOf("week").format("MMM D")} - ${dayjs(date).endOf("week").format("MMM D")}`
	if (viewMode === Views.WORK_WEEK)
		return `${dayjs(date).startOf("week").add(1, "day").format("MMM D")} - ${dayjs(date)
			.endOf("week")
			.add(-1, "day")
			.format("MMM D")}`
	if (viewMode === Views.AGENDA) return `${dayjs(date).format("L")} - ${dayjs(date).add(1, "month").format("L")}`
	return dayjs(date).format("dddd, MMM D")
}

export const getTodayButtonLabel = (viewMode: "month" | "week" | "work_week" | "day" | "agenda") => {
...
export const CalendarTodayButton: FC<ICalendarTodayButtonProps> = ({ setDate, date, unitType, viewMode }) => {
	return (
		<ButtonGroup>
			<Button
				color='info'
...
export const CalendarViewModeButtons: FC<ICalendarViewModeButtonsProps> = ({ viewMode, setViewMode }) => {
	return (
		<Dropdown>
			<DropdownToggle>
				<Button
