...
export const AlertHeading: FC<IAlertHeadingProps> = ({ className, children, tag = "h4", ...props }) => {
	return (
...
export const AlertLink: FC<IAlertLinkProps> = ({ className, children, href, to = null, ...props }) => {
	...
	if (status) {
		return (
			<div
				...
				{isDismissible && (
					<button type='button' className='btn-close' aria-label='Close' onClick={() => setStatus(false)} />
				)}
			</div>
		)
	}
