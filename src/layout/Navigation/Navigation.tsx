...
export const NavigationTitle: FC<INavigationTitleProps> = ({ className = undefined, children, ...props }) => {
	return (
	...
	function fillMenu(
		...
		return Object.keys(data).map((item) =>
			data[item].path ? (
				...
				!isMore &&
				!isHorizontal && (
					<NavigationTitle key={data[item].id}>{t(data[item].text) as ReactNode}</NavigationTitle>
				)
			),
		)
	}
	...
	return (
		...
		<nav ref={ref} aria-label={id} className={className} {...props}>
			<List id={id} horizontal={horizontal}>
				...
				{horizontal && (
					<Item rootId={`other-${id}`} title={t("More") as string} icon='MoreHoriz' isHorizontal isMore>
